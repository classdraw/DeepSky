...
namespace UnityEngine.UI.Extensions
{
	...
	public class UIParticleSystem : MaskableGraphic {
		...
		protected bool Initialize() {
			...
			if (_particleSystem == null) {
				...
				if (mainModule.maxParticles > 14000)
				{
			...
			if (_particles == null) {
				_particles = new ParticleSystem.Particle[mainModule.maxParticles];
			}
			...
			if (_textureSheetAnimation.enabled) {
				...
				if (_textureSheetAnimationFrames <= 0) {
					return false;
				}
			}
	...
	//                float frameProgress = textureSheetAnimation.frameOverTime.curveMin.Evaluate(1 - (particle.lifetime / particle.startLifetime)); // TODO - once Unity allows MinMaxCurve reading
					...
					if (_textureSheetAnimation.mode == ParticleSystemAnimationMode.Grid) {
	#endif
	...
	//                    }
	...
	#if UNITY_2017_1_OR_NEWER
					} else if (_textureSheetAnimation.mode == ParticleSystemAnimationMode.Sprites) {
						frame = Mathf.FloorToInt (frameProgress * _textureSheetAnimation.spriteCount);
						Sprite s = _textureSheetAnimation.GetSprite (frame);
						if (s != null) {
							particleUV = UnityEngine.Sprites.DataUtility.GetOuterUV (s);
						}
					}
	#endif
				}
		...
		void Update() {
			if (Application.isPlaying) {
				_particleSystem.Simulate(Global.FixedDeltaTime(), false, false,true);
				SetAllDirty();
			}
		}
