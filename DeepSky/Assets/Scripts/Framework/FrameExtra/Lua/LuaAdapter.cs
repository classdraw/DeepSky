...
// 64 bit version
[StructLayout(LayoutKind.Explicit, Size = 16)]//12 win7是16
...
public static class LuaEnvValues
{
    ...
    public const int LUA_TNUMFLT =(LUA_TNUMBER | (0 << 4));//(LUA_TNUMBER | (0 << 4));
    public const int LUA_TNUMINT = (LUA_TNUMBER | (1 << 4));//(LUA_TNUMBER | (1 << 4));
}
...
public unsafe class LuaArrAccess64 : LuaArrAccess
{
    ...
    public override int GetInt(int index)
    {
        if (TableRawPtr != null&& index > 0 && index <= TableRawPtr->sizearray)
        {
            ...
            // XLogger.LogError(((int)tv->i)+">>"+((int)tv->n));
