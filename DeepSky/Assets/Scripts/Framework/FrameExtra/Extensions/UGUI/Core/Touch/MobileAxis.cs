...
public class MobileAxis {
    ...
    public MobileAxis(string axisName)
        :this()
    {
		name = axisName;
        unityAxis = axisName;
	...
	public void UpdateAxis(float realValue, bool isOnDrag,bool deltaTime=true){
		...
		// Axis value
		ComputAxisValue(realValue,isOnDrag,deltaTime );
	...
	public void DoDirectAction(){
		...
		if (directTransform){
			...
			case MobileAxis.DirectAction.TranslateLocal:
				if ( directCharacterController==null){
			...
			case MobileAxis.DirectAction.Scale:
				directTransform.localScale +=  localAxis *  axisSpeedValue;
				break;

			case MobileAxis.DirectAction.Force:
			...
			case MobileAxis.DirectAction.RelativeForce:
				if (directRigidBody!=null){
			...
			case MobileAxis.DirectAction.Torque:
				if (directRigidBody!=null){
			...
			case MobileAxis.DirectAction.RelativeTorque:
				if (directRigidBody!=null){
			...
			case MobileAxis.DirectAction.Jump:
				if ( directCharacterController!=null){
	...
	private void ComputAxisValue(float realValue, bool isOnDrag, bool deltaTime){
	...
	private Vector3 GetInfluencedAxis(){
		...
		case MobileAxis.AxisInfluenced.X:
			axis = Vector3.right;
			break;
		case MobileAxis.AxisInfluenced.Y:
			axis = Vector3.up;
			break;
		case MobileAxis.AxisInfluenced.Z:
    ...
    public float GetUnityAxis(bool isRaw=false)
    {
        if (isRaw)
        {
			return Input.GetAxisRaw(this.unityAxis);
        }
        else
        {
			return Input.GetAxis(this.unityAxis);
        }
    }
}
